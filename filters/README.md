...
This folder contains the reusable filter utilities used in **Stage 2A** of the
SFT-Collection-v2 pipeline. The public API is grouped by the same six
sub-stages that appear in the Sankey diagram for Stage 2A:
...
- **No LLM calls** — all filters are local, deterministic, and fast.
- **Row-level** — each filter returns either `bool` or a string drop-reason.
- **Auditable** — every dropped row gets a stable `_drop_reason` label that maps
  1:1 to a phase in the Sankey diagram.
- **Inspired by OLMo-3 / OpenThoughts** — adapted from AllenAI Open-Instruct
  and the OpenThoughts3 appendix.

## Stability guarantee

The phase modules above are *facades*: they re-export the exact same function
objects as the original module files in this folder
...
`code_task_filters.py`, `fasttext_filters.py`).

This is verified at import time — each phase function satisfies
`phase_module.fn is original_module.fn`. The original modules remain the
canonical implementation, so behaviour is **byte-for-byte identical** with the
Stage 2A run that produced SFT-Collection-v2.

`phase_2a1_structural.py` is the only module containing genuinely new code:
it bundles the four structural checks (`bad_messages`, `last_not_assistant`,
`prompt_too_short`, `empty_assistant`) that were previously inlined in every
per-dataset `filter_and_format_*.py` script. The check order, defaults
(`min_prompt_chars=50`, `min_messages=2`), and drop-reason labels match the
legacy scripts exactly.

## How filters are used by pipeline scripts

Pipeline scripts (`../filter_and_format/*.py`) typically:

1. Load a raw dataset from disk: `datasets.load_from_disk()`
2. `map()` each row, calling these filters to decide keep / drop
3. Set `_keep=False` and `_drop_reason="<label>"` for dropped rows
4. Filter on `_keep=True` and export both kept and dropped rows to Parquet
   (the dropped split is kept for audit / Sankey reporting)
