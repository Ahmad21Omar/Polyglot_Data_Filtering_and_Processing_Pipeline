...
This folder is the **shared** Stage 2A pipeline used by every source dataset
in SFT-Collection-v2. There is no Dolci-specific template — Dolci is just one
of 13 adapters under [`adapters/`](adapters/).
...
├── LANGUAGE_HANDLING.md   ← per-dataset language filter decisions (read this first)
...
[`analytics/sft/Filtering_process/plot_step2a_filtering.py`](../../analytics/sft/Filtering_process/plot_step2a_filtering.py)
1:1. The legacy per-dataset scripts that produced SFT-Collection-v2 had a
slightly different ordering (e.g. ran the question filter before
`empty_assistant`, ran `identity` before think extraction); the new pipeline
adopts the canonical Sankey order as a single source of truth. Output schema
and the *set* of drop reasons are identical to the legacy scripts; absolute
drop counts may differ marginally for rows that would have been caught by an
earlier phase under the canonical order.
...
```python
from Filtering_Pipeline.filter_and_format import (
    english_only_preset, run,
)
from Filtering_Pipeline.filter_and_format.adapters.dolci_think_sft_7b import (
...
```python
from Filtering_Pipeline.filter_and_format.adapters.openthoughts3 import OpenThoughts3Adapter
from Filtering_Pipeline.filter_and_format.adapters.nemotron_math_proofs_v1 import NemotronMathProofsV1Adapter
```
...
Add the file under [`adapters/`](adapters/) and you're done — the pipeline,
schema, filters and CLI are shared.

## Status

All 12 adapters are implemented and pass an end-to-end smoke test (load
synthetic data → run pipeline → verify expected `_drop_reason` distribution
...
Each new adapter is followed by the same smoke-test contract: load 5 rows
from disk → run the pipeline → assert that the kept/dropped counts and the
output schema match expectations.
